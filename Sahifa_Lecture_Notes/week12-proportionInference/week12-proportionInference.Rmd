...
* Download `week12-proportionInference.Rmd` to `STAT240/lecture/week12-proportionInference`.
...
* ... the **Critical Value** (a.k.a "quantile confidence score") is the ($c$ + (1-$c$)/2) quantile of the sampling distribution... (where your confidence level is $c = 1 - \alpha$)...
...
$$
\hat{p} \pm \text{qnorm}(c+(1-c)/2) * \sqrt{\frac{\hat{p}(1-\hat{p})}{n}}
$$
...
$$
\hat{p}_{AC} \pm \text{qnorm}(c+(1-c)/2) * \sqrt{\frac{\hat{p}_{AC}(1-\hat{p}_{AC})}{n+4}}
$$
...
> The Agresti-Coull adjustment, **in practice**, usually leads to intervals with HIGHER than $c = 1-\alpha$ coverage probability, especially for extreme $\hat{p}$. The Wald adjustment **in practice** usually leads to lower than $c = 1-\alpha$ coverage probability, especially for extreme $\hat{p}$.
...
* The sampling distribution of $\hat{p}$ is normal, so the quantile confidence score is `qnorm(c + (1-c)/2)`. 
    - For the common confidence level 95% ($\alpha = 0.05$), this value is `qnorm(1-alpha + alpha/2) = qnorm(0.975) = 1.96`.
...
## Step 4: Identify p-value from Data.
...
> Just like its single proportion equivalent, this "A-C" adjustment adds two successes and two failures to the dataset; but for two proportions, we **split them evenly amongst the two groups.**
...
$$
\hat{p}_{1AC} - \hat{p}_{2AC} \pm \text{qnorm(c + (1-c)/2)} * \sqrt{ \frac{\hat{p}_{1AC}(1-\hat{p}_{1AC})}{n_1+2} + \frac{\hat{p}_{2AC}(1-\hat{p}_{2AC})}{n_2+2} }
$$
