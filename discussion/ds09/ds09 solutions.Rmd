...
> Yes, we can see that the confidence interval doesn't cover 0.5 and covers values below 0.5, indicating that the actual value of p is below 0.5.
...
**(b)** Assuming that the probability of getting head while flipping the unfair coin from problem 1 is 0.35. What will be the distribution of the sample means for part (a)?
...
```{r}
pnorm(sample_mean, 100*0.35, sqrt(100*0.35*0.65/5))
```
...
> Linearity: the residual almost behave non linearly but still can be considered linear. 

> Normal with mean 0: There's a good chance the mean of this is not zero.

> Constant variance: The variance seems to reduce as the x increases.
...
> Normal with mean zero: Most of the points are close to the line, with a few points scattered away from the line in a symmetric way. So, the residuals appear to be normal around zero.
