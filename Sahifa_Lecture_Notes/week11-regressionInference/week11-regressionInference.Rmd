...
* Our **critical value** ("quantile confidence score") is the $C + \frac{1-C}{2}$ quantile of the **sampling distribution** (which we don't know yet).
...
$$
SE(\hat{\beta}_1) = \sqrt{\frac{\sum_{i = 1}^n (y_i - \hat{y}_i)^2/(n-1)}{\sum_{i = 1}^n (x_i - \overline{x})^2}}
$$
...
* Putting this into practice more formally, every hypothesis test has five steps.
...
$H_0$ should be a statement about a parameter with $=$, and $H_{\alpha}$ should be another statement about that same parameter with $>$, $<$, or $\neq$.
...
## Step 4: Identify Outcomes from Data and Calculate P-Value

* From the data, we calculate a value of the test statistic.

> If $H_{\alpha}$ is $>$, the relevant outcomes are **starting from the observed test statistic and everything to its right**.

> If $H_{\alpha}$ is $<$, the relevant outcomes are **starting from the observed test statistic and everything to its left**.
...
> The **p-value** is the total probability as extreme as **the outcomes** on **the null distribution from step 3**.
...
* In fact, the test statistic here takes on a very common and intuitive form that we will see multiple times in this class: $$\frac{\text{Point Estimate - Parameter_{H_0}}}{\text{Standard Error}},$$

where, $Parameter_{H_0}$ is the value of parameter under the null hypothesis ($H_0$).
...
* Recall that a small p-value constitutes strong evidence for $H_{\alpha}$ in context.
...
$$
s_{\hat{y}^*} = \sqrt{((n-2)^{-1} \sum_{i=1}^n(y_i - \hat{y}_i)^2)(1 + n^{-1} + \frac{(x^* - \overline{x})^2}{\sum_{i = 1}^n (x_i - \overline{x})^2} )}
$$
...
* Our Critical Value is still `qt(C + (1-C)/2, df = n - 2)`.
