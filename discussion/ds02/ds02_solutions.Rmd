...
-   This assignment should be completed in your **discussion groups**.
...
> 1:10 represents the sequence 1, 2, 3, ... 10, and similarly for 11:20 representing 11, 12, 13, ..., 20. These sequences are combined by adding the first element of each (1 + 11 = 12), then the second of each (2 + 12 = 14), the third of each (3 + 13 = 16), and so on, until 10 + 20 = 30.
...
> There is at least one NA value in logicalValues, which turns the whole computation to NA.

b)  **Explain** what numerical values `TRUE` and `FALSE` represent when doing arithmetic.

> TRUE represents 1 and FALSE represents 0 in arithmetic.
...
> na.rm = TRUE means to "ignore" the NA value when computing the mean. The values of logicalValues are not affected permanently, it still has the NA in it, it is just ignored for the computation..
...
is.na(sw_heights) & is.na(sw_mass)
```
...
>  There are 28 Star Wars characters in this dataframe whose mass is missing/unknown.
...
> That's no error - it's Jabba the Hutt!
