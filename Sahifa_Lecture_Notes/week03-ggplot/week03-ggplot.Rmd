...
- The plots that we saw above are helpful for understanding the *relationship between two numeric variables* (`duration` and `year1`) is reflected in what aesthetics the `geom`s require.
    + `geom_point`, `geom_line`, and `geom_smooth` all require an `x` AND a `y`.
    
### geom_point, geom_line and geom_path

> These are the three most common geom functions used. `geom_point` is used to create point graphs, `geom_line` or `geom_path` connect the data values in order of the variable on the x axis or in the data, respectively and gives you a line graph. 
...
- Just like we have already seen before you can regulate the size, shape (for `geom_point` using the `shape` argument) or type (for `geom_line` using the `linetype` argument) and color of these plots.
...
- You can obtain different type of trendlines using the argument `method`. The default is the `method = "loess` that gives the best fit line (non-linear).  If you would rather have the best *straight* line through the data (more on that in the last week of the course!) you can set `method = "lm"`.
...
> This is a two variable plot that takes in one categorical variable and one numeric variable, and creates one bar for each row in the dataframe, with the heights specified by that numeric variable.
...
* With `geom_col`, you provide the heights of the bars. This can be both a strength and a weakness.
    ...
    + If you do want to plot number of occurrences or count in the y-axis, `geom_col` requires you to do the calculation of the heights and put that in as the `y` variable.
    
* To understand this plot, we'll need to create a `century` column, which is either "19" for the 1800's, "20" for the 1900's, or "21" for the 2000's. (We'll also continue to use this variable through the rest of the file.)
...
> The following `geom`s analyze the distribution of a single variable, and as such, they require **exactly one** of `x` or `y`. They will then compute some useful statistic to serve as the one you don't give.
...
* Notice how this data is different from the data we passed into `geom_col`. The data for `geom_col`, had one row per visual bar. The data above for `geom_bar`, has way more rows in the dataset than we will eventually have bars on the graph.
...
> There are three functions to create bar graphs; `geom_histogram`, `geom_bar` and `geom_col`. 

- `geom_histogram` is used to plot occurrences of continuous random variable and helps understand the distribution of those continuous variables (similar to what `geom_density` does).

- `geom_bar` is a shortcut to plot occurrences of categorical/discrete variable and is the only way to understand the distribution of those variables.

- `geom_col` is a more general two-variable bar graph function that can help you visualize different types of statistics with respect to groups or categories created.
...
* The bar graph, at the end of the `geom_col` section and the scatter plot with `geom_smooth()` can be used to address this question:
...
For those interested in learning to use ggplot for more beautiful and complex plots you can refer to the [book in this link](https://ggplot2-book.org) 
