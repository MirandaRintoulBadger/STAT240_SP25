...
We will use `ross` to investigate some interesting inference questions.
...
Is there an association between the appearance of a man-made structure and the appearance of grass in a painting? Consider two populations: paintings in our hypothetical population with and without grass.
...
* Conduct a hypothesis test to assess the evidence for a difference in true average number of colors used in Bob Ross TJOP paintings with and without grass.  Use $\alpha = 0.05$ and interpret your results in context.
...
* Conduct a hypothesis test for a POSITIVE linear relationship between the number of features (as Y) in a painting and the number of colors (as X).  Code to create a "number of features" column as well as a scatterplot is provided for you.
...
ggplot(lm_data, aes(x = num_colors, y = num_features)) +
