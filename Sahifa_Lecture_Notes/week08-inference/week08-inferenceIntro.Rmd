...
* The general adjustment here is that if your confidence level is $c= 1-\alpha$, then the right endpoint is the $c + \frac{1-c}{2}$ quantile of the sampling distribution. (Start at $c$, and then go halfway towards 1.)
...
> The **null** hypothesis is usually of the form $\text{parameter} = \text{Value}$. It is the thing we wish to **attack** or **disprove**. These null hypothesis with an equality are what we call **simple null hypothesis**. We also have **composite null hypothesis** where the usual form of the null hypothesis is as $\text{parameter} \text{or} \geq \text{or} \leq \text{Value}$

> The **alternative** hypothesis is of the form $\text{parameter} \neq \text{or} > \text{or} < \text{Same Value As Null}$. It is the thing we wish to **show is likely true**. 

* Simple or composite null hypothesis, every hypothesis testing focuses on the alternative hypothesis, so the method of inference doesn't change because of the type of null hypothesis.
