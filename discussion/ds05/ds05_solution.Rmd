...
#billboard_weekly %>% filter(week == "wk1") %>% slice_min(rank, n = 1)
```
...
#billboard_weekly %>% distinct(artist, track) %>% count(track) %>% filter(n > 1)
```
...
#or inner_join(longest_running_songs, billboard_weekly)

#or left_join(billboard_weekly, longest_running_songs) %>% slice_max(date.of.week, n = 3)

#or left_join(billboard_weekly, longest_running_songs) %>% drop_na(date.of.week)
```
...
# billboard_weekly %>% filter(week %in% c("wk1", "wk5")) %>% group_by(artist, track) %>% summarize(rise = diff(rank)) %>%ungroup() %>% slice_max(rise, n = 1)
```
