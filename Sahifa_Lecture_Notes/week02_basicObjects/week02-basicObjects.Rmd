...
### Loading A Dataframe into RStudio

* `alphabet` was relatively small. Let's look at a larger dataframe with information about Lake Mendota freezing.
...
### Some useful functions to use with dataframes

- We can view the data set after loading it onto RStudio by using the function `View(name_of_the_dataset)` or by simply clicking the name of the dataset in the environment. 

```{r}
View(mendota)
```

- If you don't want to see the whole dataset but instead just the first few or the last few rows of it then you can use the functions `head(name_of_the_dataset)` or `tail(name_of_the_dataset)`. The `head()` and `tail()` functions by default show the first or last 6 rows of the dataset, if you want to view a custom number of rows then you can enter that number as the second argument in the functions.
...
- We can see the type of each column with the function `glimpse(name_of_the_dataset)`. This will list down all the column names and there types.

```{r}
glimpse(mendota)
```

- The dimension of the dataset $[row, col]$ can be obtained using the function `dim(name_of_the_dataset)`, you can obtain the number of columns and number of rows seperately using the functions `nrow(name_of_the_dataset)` and `ncol(name_of_the_dataset)`.
...
There are more functions that you can explore and find.
