...
* Now that we know $P(X = x) = \binom{n}{x} * p^x * (1-p)^{n-x}$ for any $X \sim Binom(n, p)$, we can plug this into our mean and variance formulas. 

* For example, to derive the mean or variance of $Binom(n, p)$, we can compute:

$$
E(X) = \sum_{k = 0}^n x * \binom{n}{x} * p^x * (1-p)^{n-x}
$$

$$
Var(X) = \sum_{k = 0}^n (x-E(x))^2 * \binom{n}{x} * p^x * (1-p)^{n-x}
$$
...
+---------------+---------------------+----------------------------------------+
| ```p<dist>``` | A value of $X$, `q` | $P(X \leq q)$                           |
+---------------+---------------------+----------------------------------------+
| ```q<dist>``` | A probability, `p`  | $q$, such that $P(X \leq q) = p$     
+---------------+---------------------+----------------------------------------+
...
* Similar to how we substituted $P(X=x)$ into the discrete mean formula to find the mean of a binomial distribution, let's subtitute $f(x)$ into the continuous mean formula to find the expression mean and variance of the normal distribution.

$$
E(X) = \int x * \frac{1}{\sigma\sqrt{2\pi}} e^{-\frac{1}{2}\left(\frac{x-\mu}{\sigma}\right)^2}dx
$$

$$
Var(X) = \int (x-\mu)^2 * \frac{1}{\sigma\sqrt{2\pi}} e^{-\frac{1}{2}\left(\frac{x-\mu}{\sigma}\right)^2}dx
$$

* That's a doozy. If you're curious, [here's the algebra behind working the E(X) out.](https://statproofbook.github.io/P/norm-mean.html#)
